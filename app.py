...
if uploaded_file:
    ...
    # Scale for mobile stability - FIXED: Better scaling
    display_width = min(1000, w)  # Don't exceed original width
    display_height = int(h * (display_width / w))
    img_resized = img.resize((display_width, display_height), Image.Resampling.LANCZOS)

    # FIXED: Added display_toolbar=True for better UX
    canvas_result = st_canvas(
        ...
        display_toolbar=True,  # Added this
    )
    ...
    if canvas_result.json_data:
        ...
        if len(objects) > 0:
            ...
            # FIXED: Handle case when image_data is None
            if canvas_result.image_data is not None:
                ...
                st.sidebar.warning("Draw something on the canvas first")
    else:
        st.sidebar.info("Click 'Finish Drawing' after drawing")
